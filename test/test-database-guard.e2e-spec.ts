...
describe('test database guard', () => {
  const validTestUrl =
    'postgresql://user:pass@localhost:5432/engmasterai_test?schema=public';
  ...
  it('rejects the development database by exact name, even under NODE_ENV=test', () => {
    const devUrl =
      'postgresql://user:pass@localhost:5432/engmasterai?schema=public';
    expect(() => assertTestDatabaseUrl(devUrl, 'test')).toThrow(
  ...
  it('rejects any database whose name does not end in "_test"', () => {
    const otherUrl =
      'postgresql://user:pass@localhost:5432/engmasterai_staging?schema=public';
    expect(() => assertTestDatabaseUrl(otherUrl, 'test')).toThrow(
      /must end in "_test"/i,
    );
  });

  it('rejects NODE_ENV values other than "test", even against a correctly named database', () => {
    expect(() => assertTestDatabaseUrl(validTestUrl, 'development')).toThrow(
      /NODE_ENV must be "test"/i,
    );
    expect(() => assertTestDatabaseUrl(validTestUrl, undefined)).toThrow(
      /NODE_ENV must be "test"/i,
    );
  });

  it('rejects a missing DATABASE_URL with an actionable message', () => {
    expect(() => assertTestDatabaseUrl(undefined, 'test')).toThrow(
  ...
  it('never includes the database URL (credentials) in its error message', () => {
    const devUrl =
      'postgresql://secretuser:secretpass@localhost:5432/engmasterai?schema=public';
    try {
...
describe('testFixtureName', () => {
  it('always starts with the shared fixture prefix, so the global sweep can find it', () => {
    expect(testFixtureName('Progress Deck A')).toMatch(
      new RegExp(
        `^${TEST_FIXTURE_PREFIX.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')} Progress Deck A `,
      ),
    );
  });

  it('produces a distinct name on every call, so repeated runs never collide', () => {
    expect(testFixtureName('Progress Deck A')).not.toBe(
      testFixtureName('Progress Deck A'),
    );
  });
});

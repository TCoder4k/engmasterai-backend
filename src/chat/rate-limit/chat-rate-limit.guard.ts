...
import { CHAT_RATE_LIMITS_KEY, ChatRateLimitPolicy } from './chat-rate-limits.decorator';

interface RequestWithUser extends Request {
  user?: { userId: string };
}

// Mirrors DictionaryRateLimitGuard/VocabDeckRateLimitGuard/
// LearningRateLimitGuard verbatim — module-local rate-limit guards are this
// codebase's convention rather than one shared implementation.
@Injectable()
export class ChatRateLimitGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    const policy = this.reflector.get<ChatRateLimitPolicy | undefined>(
      CHAT_RATE_LIMITS_KEY,
      context.getHandler(),
    );
    ...
    const key = `chat:${policy.kind}:${userId}`;

...
export class MediaResolverService {
  ...
  resolve(words: ImportWord[], config: DatasetConfig, runId: string): MediaManifest {
    ...
    for (const word of words) {
      ...
      if (config.media?.audio) {
        this.pushEntry(entries, seen, 'audio', word, textKey, deckKey, config.id, config.media.audio, AUDIO_EXTENSIONS);
      }
      if (config.media?.image) {
        this.pushEntry(entries, seen, 'image', word, textKey, deckKey, config.id, config.media.image, IMAGE_EXTENSIONS);
      }
    }
  ...
  private pushEntry(
    ...
    const localPath = match.matchType !== 'none' ? resolveMediaPath(dir, match.fileName!) : undefined;
